"""Unit tests for the three-way tsmom/ms-shift/tsmom-tlt-gld blend: per-leg
column routing, the blend arithmetic on the shared SPY column, weight-sum
validation, output bounds, and lookahead safety through the harness guard."""
...
_spec = importlib.util.spec_from_file_location(
    "tsmom_ms_shift_tltgld_blend",
    Path(__file__).resolve().parents[1] / "strategies/tsmom_ms_shift_tltgld_blend.py",
)
...
def test_qqq_column_is_pure_tsmom_plus_ms_shift_no_tltgld_leak():
...
def test_tlt_and_gld_columns_are_pure_tlt_gld_leg_no_leak():
    bars = bars_from_ohlc(400, UNIVERSE, seed=2)
    sig = blend_mod.Signal()
    out = sig.generate(bars)
    c = sig._tlt_gld.generate(blend_mod._subset(bars, blend_mod.TLT_GLD_COLS))
    for col in ("TLT", "GLD"):
        expected = sig.weight_tlt_gld * c[col]
...
def test_spy_column_sums_all_three_legs():
    ...
    c = sig._tlt_gld.generate(blend_mod._subset(bars, blend_mod.TLT_GLD_COLS))
    expected = (
        sig.weight_tsmom * a["SPY"] + sig.weight_ms_shift * b["SPY"]
    )
    assert np.allclose(out["SPY"].to_numpy(), expected.to_numpy())
    assert "SPY" not in blend_mod.TLT_GLD_COLS  # sanity: third leg never touches SPY
...
def test_weights_must_sum_to_one():
    with pytest.raises(ValueError):
        blend_mod.Signal(weight_tsmom=0.5, weight_ms_shift=0.5, weight_tlt_gld=0.5)


def test_negative_weight_raises():
    with pytest.raises(ValueError):
        blend_mod.Signal(weight_tsmom=-0.1, weight_ms_shift=0.6, weight_tlt_gld=0.5)
