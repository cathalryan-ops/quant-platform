"""Unit tests for the tsmom/ms-shift blend: the blend arithmetic itself
(both components' outputs land where expected on constructed cases),
blend_weight edge cases (0.0 and 1.0 reduce to a pure single-leg signal),
...
_spec = importlib.util.spec_from_file_location(
    "tsmom_ms_shift_blend",
    Path(__file__).resolve().parents[1] / "strategies/tsmom_ms_shift_blend.py",
)
...
def bars_from_ohlc(n: int, seed: int = 0) -> BarFrame:
    ...
    mk = lambda arr: pd.DataFrame({"SPY": arr}, index=idx)
...
def test_default_blend_is_the_average_of_both_legs():
    ...
    a = sig._tsmom.generate(bars)
    b = sig._ms_shift.generate(bars)
    expected = 0.5 * a["SPY"] + 0.5 * b["SPY"]
    assert np.allclose(out["SPY"].to_numpy(), expected.to_numpy())


def test_blend_weight_one_reduces_to_pure_tsmom():
    bars = bars_from_ohlc(400, seed=2)
    sig = blend_mod.Signal(blend_weight=1.0)
    out = sig.generate(bars)
    a = sig._tsmom.generate(bars)
    assert np.allclose(out["SPY"].to_numpy(), a["SPY"].to_numpy())


def test_blend_weight_zero_reduces_to_pure_ms_shift():
    bars = bars_from_ohlc(400, seed=3)
    sig = blend_mod.Signal(blend_weight=0.0)
    out = sig.generate(bars)
    b = sig._ms_shift.generate(bars)
    assert np.allclose(out["SPY"].to_numpy(), b["SPY"].to_numpy())
