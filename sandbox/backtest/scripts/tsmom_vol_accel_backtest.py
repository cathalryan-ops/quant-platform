"""First real backtest of tsmom-vol-accel (see
brain/wiki/strategies/tsmom-vol-accel.md), through the real
...
Also runs the same falsification check as tsmom_backtest.py (flat fraction
during the 2020 COVID crash and 2022 bear market windows) and prints it
directly against tsmom-spy-qqq's recorded numbers (0.0% flat through
COVID on both symbols) -- the specific thing this page exists to test.
"""
...
# tsmom-spy-qqq's recorded falsification-check numbers (see
# brain/wiki/strategies/tsmom-spy-qqq.md's Evidence section).
TSMOM_COVID_FLAT = {"SPY": 0.0, "QQQ": 0.0}
TSMOM_2022_FLAT = {"SPY": 0.48, "QQQ": 0.52}
...
def main() -> None:
    ...
    manifest = StrategyManifest.model_validate(
        {
            ...
            "universe": ["SPY", "QQQ"],
            "hypothesis": (
                "tsmom-spy-qqq's unchanged signal, gated off whenever 5-day realized "
                "vol expands past 1.75x the 63-day realized-vol baseline, should "
                "reduce or eliminate the COVID-crash blind spot (0% flat time in "
                "tsmom-spy-qqq) while preserving walk-forward Sharpe; killed if Sharpe "
                "does not clear 1.0, or if COVID-window flat time does not increase "
                "materially over tsmom-spy-qqq's 0.0%."
            ),
            "signal_spec": {
                "language": "python",
                "entrypoint": "strategies/tsmom_vol_accel.py:Signal",
            },
            "risk": {
    ...
    print(f"{'window':>16} {'sym':>4} {'tsmom-vol-accel':>16} {'tsmom-spy-qqq (recorded)':>26}")
    for label, (start, end), baseline in (
        ("COVID crash", COVID_CRASH, TSMOM_COVID_FLAT),
        ("2022 bear market", BEAR_2022, TSMOM_2022_FLAT),
    ):
        window = weights.loc[start:end]
        for sym in manifest.universe:
            flat_frac = float((window[sym] == 0.0).mean())
            print(f"{label:>16} {sym:>4} {flat_frac:>15.1%} {baseline[sym]:>25.1%}")
