"""First real backtest of tsmom-ms-shift-tltgld-blend (see
brain/wiki/strategies/tsmom-ms-shift-tltgld-blend.md), through the real
engine.py/vectorbt path. Uses the 16-symbol wider-universe snapshot
restricted to SPY/QQQ/TLT/GLD.
...
tsmom-spy-qqq alone, ms-shift-spy-high-displacement alone, tsmom-tlt-gld
alone, and the 3-way blend -- apples-to-apples fresh re-runs, same
discipline as tsmom_ms_shift_dualmom_blend_backtest.py, for a direct
comparison against that script's dual-momentum-leg result (Sharpe
0.863027, below the 2-leg blend's 0.884266).
"""
...
def main() -> None:
    ...
    legs = [
        ...
        (
            "tsmom-tlt-gld-refresh",
            ["TLT", "GLD"],
            "strategies/tsmom_spy_qqq.py:Signal",
            "Fresh current-engine re-run of tsmom-tlt-gld for apples-to-apples comparison.",
        ),
        (
            "tsmom-ms-shift-tltgld-blend",
            FULL_UNION_UNIVERSE,
            "strategies/tsmom_ms_shift_tltgld_blend.py:Signal",
            (
                "Extending tsmom-ms-shift-blend (Sharpe 0.884266) with tsmom-tlt-gld as a third leg "
                "at equal 1/3 weight (fixed a priori), on the strength of its near-zero correlation "
                "to both existing legs. Killed if walk-forward Sharpe does not clear the 2-leg "
                "blend's own current-engine Sharpe or the standing 1.0/1.2 gate."
            ),
        ),
    ]
    ...
    pairs = [
        ...
        ("tsmom-spy-qqq-refresh", "tsmom-tlt-gld-refresh"),
        ("ms-shift-spy-v2-refresh", "tsmom-tlt-gld-refresh"),
    ]
    for x, y in pairs:
        a = returns_by_leg[x]
        b = returns_by_leg[y]
